...
aws cloudformation --region $region describe-stacks --stack-name $stack_name --profile pgx > /dev/null 2>&1
retVal=$?
...
do
  status=$(aws cloudformation --region $region describe-stacks --stack-name $stack_name --query "Stacks[0].StackStatus" --output text --profile pgx)
...
outputs=$(aws cloudformation --region $region describe-stacks --stack-name $stack_name --query "Stacks[0].Outputs" --output text --profile pgx)
...
aws ec2 --region $region revoke-security-group-egress --group-id $engine_sg --protocol all --port all --cidr 0.0.0.0/0  --profile pgx > /dev/null 2>&1
...
printf "aws emr describe-studio --region $region --studio-id $studio_id --profile pgx"
printf "\n"

echo "To list all of the Studios in the specified Region, use:"
printf "aws emr list-studios --region $region --profile pgx"

printf "\n"
echo "To delete the Studio, use:"
echo "aws emr delete-studio --region $region --studio-id $studio_id --profile pgx"
printf "\n"
...
echo "aws emr create-studio-session-mapping --region $region --studio-id $studio_id --identity-name johndoe@enterprise.com --identity-type USER --session-policy-arn $advanced_policy --profile pgx"

printf "\n"
echo "To assign a group (for example, data-org) to the Studio and attach a session policy, use:"
echo "aws emr create-studio-session-mapping --region $region --studio-id $studio_id --identity-name data-org --identity-type GROUP --session-policy-arn $advanced_policy --profile pgx"
