#!/usr/bin/env python3
"""
Run Athena row counts on medical bronze/silver/gold and verify:
  - Silver = Gold (no data loss during ICD code standardization).
  - Gold <= Bronze (no unexpected row inflation).

Exits 0 if checks pass, 1 otherwise. Per-layer databases: bronze_medical, silver_medical, gold_medical.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate medical bronze/silver/gold row coverage; exit 0 only if silver=gold and gold<=bronze."
    )
    ...
    counts = {}
    for logical_name, s3_prefix, _suggested, database_name in MEDICAL_LAYERS:
    ...
    b = counts.get("bronze_medical")
    s = counts.get("silver_medical_partitioned")
    g = counts.get("gold_medical")
    if None in (b, s, g):
        print("\n[FAIL] Missing row count(s); cannot validate.", file=sys.stderr)
        return 1
    ok = True
    if s != g:
        print(f"\n[FAIL] Silver ({s:,}) != Gold ({g:,}); possible data loss during ICD code standardization.", file=sys.stderr)
        ok = False
    else:
        print(f"\n[OK] Silver = Gold ({g:,}); no data loss during ICD code standardization.")
    if g > b:
